	...
	private void OnGUI()
	{
        ...
        if (worked)
        {
            ...
            for (int i = 0; i < max; i++)
			{
                ...
                {
                    if (edited.Contains(id))
                        GUILayout.Label(stringOrig[id], styleEdit, GUILayout.Width((linesize) / 2));
                    else if (origs.Contains(id))
                    {
                        if (origsinChanged[origs.IndexOf(id)] == origs.IndexOf(id))
                            GUILayout.Label(stringOrig[id], GUILayout.Width((linesize) / 2));
                        else if (!string.IsNullOrEmpty(stringOrig[id]))
                            GUILayout.Label(stringOrig[id], GUILayout.Width((linesize) / 2));
                        else
                            GUILayout.Label(stringOrig[id], GUILayout.Width((linesize) / 2));
                        ...
                        GUILayout.Label(stringOrig[id], styleAdded, GUILayout.Width((linesize) / 2));
                }
                else
                    GUILayout.Label("", styleDelete, GUILayout.Width((linesize) / 2));

                //changed
                if (stringChanged.Count > id)
                    GUILayout.Label(stringChanged[id], GUILayout.Width((linesize) / 2));
                else
                    GUILayout.Label("", GUILayout.Width((linesize) / 2));
    ...
    private void OnGUI()
    {
        if (loaded != null)
        {
            ...
            for (int i=0;i<loaded.Count;i++)
			{
                ...
                GUILayout.Label(id + "", style, GUILayout.Width(60));
                GUILayout.Label(loaded[id]);
